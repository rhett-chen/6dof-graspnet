...
class Evaluator():
    ...
    def read_eval_scene(self, file_path, visualize=False):
        ...
        return pc[:, :3], grasps, grasp_labels, {'cad_path': obj_grasp_data[-2], 'cad_scale': obj_grasp_data[-1],
                                                 'to_canonical_transformation': grasp_data_reader.inverse_transform(camera_pose)}
    ...
    def eval_scene(self, file_path, visualize=False):
        ...
        if self._eval_grasp_evaluator:
            ...
            output_grasps, output_scores, _ = self._grasp_estimator.predict_grasps(self._sess,
                pc, latents, 0, grasps_rt=grasps)
            ...
            if visualize:
                ...
                mlab.show()
                draw_scene(canonical_pc + mesh_mean, grasps=generated_grasps_canonical, mesh=mesh,
    ...
    def metric_classification_mean_ap(self, gt_labels_list, scores_list, visualize):
        ...
        np.save(
            ...
            {'cfg':self._cfg, 'precisions': precision, 'recalls': recall, 'average_precision': average_precision,
