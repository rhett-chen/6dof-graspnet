...
class GraspEvaluatorData(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        meta = {'pc': data[0][:, :, :3], 'grasp_rt': gt_control_points[:, :, :3], 'labels': data[2], 'quality': data[3],
                'pc_pose': data[4], 'cad_path': data[5], 'cad_scale': data[6]}
        return meta
    ...
    def get_uniform_evaluator_data(self, path, verify_grasps=False):
        ...
        output_cad_scales = np.asarray([cad_scale] * self.opt.batch_size, np.float32)
    ...
    def get_nonuniform_evaluator_data(self, path, verify_grasps=False):
        ...
        num_positive = int(self.opt.num_grasps_per_object * self.ratio_positive)
        ...
        negative_clusters = self.sample_grasp_indexes(num_negative, neg_grasps, neg_qualities)
        hard_neg_candidates = []
        # Fill in Positive Examples.
        for positive_cluster in positive_clusters:
            selected_grasp = pos_grasps[positive_cluster[0]][
