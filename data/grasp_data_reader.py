...
from utils.online_object_renderer import OnlineObjectRendererMultiProcess, OnlineObjectRenderer
import random
...
class PointCloudReader:
    ...
    def _get_uniform_evaluator_data(self, grasp_path, verify_grasps=False):
        ...
        for clusters, grasps, qualities in zip([positive_clusters, negative_clusters], [pos_grasps, neg_grasps],
        ...
        if grasp_path not in self._collision_hard_neg_queue or len(
                self._collision_hard_neg_queue[grasp_path]) < num_hard_negative:
            if grasp_path not in self._collision_hard_neg_queue:
